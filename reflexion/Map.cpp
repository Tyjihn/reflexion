...
std::unordered_set<int> solid_tiles =
{
    56, 57, 58, 59,    61, 62, 63, 64,    66, 68, 69,
    70, 71, 73,        75, 76, 77, 78,    80, 81, 82,
    84, 85, 86, 87,    89, 90, 91, 92,    95, 96, 97,
    99, 100, 101,     103,104,105,106,   108,109,111,
    126, 127, 128, 129,
    140, 141, 142, 143,
    154, 155, 156, 157,
    182, 183, 184, 185,
...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (solid_tiles.find(tile) == solid_tiles.end()) return false;
    ...
    *penetration_y = (m_tile_size / 2) - fabs(position.y - tile_center_y);
    //*penetration_x = glm::max(0.0f, (m_tile_size / 2) - fabs(position.x - tile_center_x));
    //*penetration_y = glm::max(0.0f, (m_tile_size / 2) - fabs(position.y - tile_center_y));
